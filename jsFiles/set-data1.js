...
for (let key in data.workHistory) {
  ...
  if (
    data.workHistory[key]["role"] !== "" &&
    data.workHistory[key]["date_start"] !== "" &&
    data.workHistory[key]["date_end"] !== "" &&
    data.workHistory[key]["location"] !== "" &&
    data.workHistory[key]["description"] !== ""
  ) {
    ...
    let workHistoryDiv = document.getElementById("work-history");
    workHistoryDiv.appendChild(workDiv);
  }
}
...
document.getElementById("telephone").innerHTML =
  "&nbsp;&nbsp;" +
  `(${data.number.slice(0, 3)}) ${data.number.slice(3, 6)}-${data.number.slice(
    6,
    10
  )}`;
